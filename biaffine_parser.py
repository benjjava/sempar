...
"""
    ...
    def forward(self, w_id_seqs, l_id_seqs, p_id_seqs, bert_tid_seqs, bert_ftid_rkss, b_pad_masks, b_fram_mat, lengths=None):
        ...
        # mode cat : avant mlp de specialisation
        #            concat de la couche cachée du MLPframe avec la repres recurrente
        elif self.stack == 'cat':
            ...
            if val_data:
                ...
                if self.dyn_weighting:
                    for stream in [sys.stdout, log_stream]:
                        stream.write(" dynamic weighting, frame= %d\n, unlab=   , lab=   " % epoch)
    ...
    def log_train_hyper(self, outstream):
        for h in ['w_emb_size', 'l_emb_size', 'p_emb_size', 'lstm_h_size','mlp_arc_o_size','mlp_arc_dropout','mlp_lab_o_size','mlp_lab_dropout','mlp_frame_h_size', 'beta1','beta2','lr','use_pretrained_w_emb', 'stack']:
