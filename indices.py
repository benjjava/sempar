...
class Indices:
    ...
    def __init__(self, known_sentences, w_embeddings_file=None, bert_tokenizer=None):
        """
        Input:
        ...
        if w_embeddings_file is not None:
            # add indices for special symbols only: calling index_new_vocab with an empty sequence of tokens
            self.index_new_vocab('w', [], add_pad=True, add_unk=True, add_root=True)
        ...
        #print("lab ok")
        self.index_new_vocab('frame', frames, add_pad=True, add_unk=False)
        #print("frame ok")
        

    def index_new_vocab(self, vocab, symbol_seq, add_pad=True, add_unk=True, add_root=False):
        """
        Input:
    ...
    def convert_frame_tok_to_indices(self, tok):
        """ 
        Input = one token from dep graph (= a 5-tuple, with list of labels / heads)
        Output: same but converted to indices for each vocab type
        """
        return [self.s2i('w', tok[0]),
                ...
                tok[3], # heads are integers already
                [ self.s2i('label', x) for x in tok[4]],
                [ self.s2i('frame', x) for x in tok[5]]]
    ...
    def convert_frame_symbols_to_indices(self, sentences):
        return [ [self.convert_frame_tok_to_indices(tok) for tok in sent] for sent in sentences ]
    ...
    def lex_dropout_itok(self, itok, dropout_rate, drop_to_unk_rate=0.01):
      ...
      for i in [0,1,2]:
      #if (random() < dropout_rate) and (len(itok)<6 or not(itok[5])):
    ...
    def load_embeddings_from_scratch(self, embeddings_file):
        ...
        for s in self.vocabs['w']['i2s']:
            ...
            elif s == PAD_SYMB:# or s == DROP_SYMB:
                # null vector for pad token
                self.iw2emb.append(np.zeros(self.w_emb_size))
            elif s == "<root>":
                # ones vector for pad token
                self.iw2emb.append(np.ones(self.w_emb_size))
    ...
    def bert_encode(self, sentences):
        ...
        for sent in sentences:
            (forms, lemmas, tags, heads, labels, frames) = list(zip(*sent))
